...
# passed the smoke tests — so "update to the
...
# breaking by definition (an application platform jump, a changed variable
...
COMPOSE_FILE="webrcade-traefik-letsencrypt-docker-compose.yml"
PROJECT="${COMPOSE_PROJECT_NAME:-webrcade}"
DRY_RUN=false
ALLOW_MAJOR=false
for arg in "$@"; do
  case "$arg" in
...
if [ "$new_major" != "$cur_major" ] && [ "$ALLOW_MAJOR" != "true" ]; then
  ...
  echo "  https://github.com/heyvaldemar/webrcade-traefik-letsencrypt-docker-compose/releases/tag/$latest" >&2
  exit 3
fi
