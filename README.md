...
Every release from v1.0.1 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `webrcade-traefik-letsencrypt-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `webrcade-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `webrcade-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob webrcade-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --bundle webrcade-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/webrcade-traefik-letsencrypt-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact webrcade-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --provenance-path webrcade-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/webrcade-traefik-letsencrypt-docker-compose
```
